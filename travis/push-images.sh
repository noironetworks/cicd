...
if [[ "$TRAVIS_TAG" =~ $RC_REGEX ]]; then
    ...
    docker tag ${BUILT_IMAGE} ${QUAY_NOIRO_REGISTRY}/${IMAGE}:${OVERRIDE_TAG}
    docker push ${QUAY_NOIRO_REGISTRY}/${IMAGE}:${OVERRIDE_TAG}
fi
...
if [[ "$TRAVIS_TAG" =~ $RC_REGEX ]]; then
    ...
    docker tag ${BUILT_IMAGE} ${DOCKER_REGISTRY}/${IMAGE}:${OVERRIDE_TAG}
    docker push ${DOCKER_REGISTRY}/${IMAGE}:${OVERRIDE_TAG}
fi